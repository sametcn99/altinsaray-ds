import { title } from "process";

export const texts = {
  title: "Altın Saray Wedding Hall",
  catchword: "Özel günlerinizi unutulmaz kılmak için.",
  address: "Harbiye, Zarif Sk. No:1 D:C, 06460 Çankaya/Ankara",
  telephone: "+90 535 457 83 12",
  whatsapp: "+905354578312",
  website: "https://altinsarayweddinghall.com/",
  email: "altinsarayweddinghall@gmail.com",
  hakkimizda: {
    title: "Hakkımızda",
    description:
      "Altın Saray Wedding Hall, büyüleyici atmosferi ve modern olanaklarıyla dikkat çekiyor. 400 kişiye kadar misafir ağırlayabilen bu mekan, özel LED ışıklandırma sistemi ve zarif dekorasyonuyla unutulmaz bir düğün deneyimi sunuyor. Kişiye özel hizmetleri ile Altın Saray Wedding Hall, Ankara'da evlenecek çiftler için rüya gibi bir ortam sağlıyor.",
  },
  ulasim: {
    title: "Ulaşım",
    description:
      "Ankara'nın her yerinden salonumuza ulaşım için Kızılay'dan Dikmen yönüne giden otobüse binip Zahit Kotku durağında inebilirsiniz. Özel aracınızla gelmek isterseniz dikmen caddesine çıkıp Harbiye sokak üzerinde bulunan Altın Saray Wedding Hall'a ulaşabilirsiniz. Detaylı ulaşım bilgisi için her zaman bizimle iletişime geçebilirsiniz.",
  },
  iletisim: {
    title: "İletişim",
    description:
      "Altın Saray Wedding Hall, düğün, nişan, kına gecesi, sünnet düğünü ve özel etkinlikleriniz için hizmet vermektedir. Rezervasyon ve fiyat bilgisi almak için bizimle iletişime geçebilirsiniz. Ayrıca, salonumuzu ziyaret etmek ve daha fazla bilgi almak için aşağıdaki iletişim bilgilerimizi kullanabilirsiniz.",
  },
  galeri: {
    title: "Galeri",
    description:
      "Altın Saray Wedding Hall'un büyüleyici atmosferini ve sunduğumuz hizmetleri daha yakından görmek için galerimize göz atın. Özel anlarınız için ilham alabileceğiniz fotoğraflar burada.",
  },
};
